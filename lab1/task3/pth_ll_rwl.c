...
#  endif
    ...
    GET_TIME(finish);
    printf("\nCustom RWLock - Elapsed time = %e seconds\n", finish - start);
    printf("Custom RWLock - Total ops = %d\n", total_ops);
    printf("Custom RWLock - member ops = %d\n", member_count);
    printf("Custom RWLock - insert ops = %d\n", insert_count);
    printf("Custom RWLock - delete ops = %d\n", delete_count);
...
void* Custom_Thread_work(void* rank) {
    ...
    for (i = 0; i < ops_per_thread; i++) {
        ...
        val = my_rand(&seed) % MAX_KEY;
        if (which_op < search_percent) {
            custom_rdlock(&custom_rwlock);
            Member(val);
            custom_unlock(&custom_rwlock);
            my_member_count++;
        } else if (which_op < search_percent + insert_percent) {
            custom_wrlock(&custom_rwlock);
            Insert(val);
            custom_unlock(&custom_rwlock);
            my_insert_count++;
        } else { /* delete */
